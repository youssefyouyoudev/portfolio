...
export function ContactForm() {
  ...
  return (
    <form action={submit} className="grid gap-4 rounded-3xl border border-cyan-400/15 bg-slate-900/60 p-5 shadow-[0_24px_80px_rgba(2,8,23,.35)] backdrop-blur-xl md:p-6">
      ...
      <label className="grid gap-2 text-sm font-medium text-slate-200">Timeline<select name="timeline" className="field">{timelines.map((item) => <option key={item}>{item}</option>)}</select></label>
